...
export interface ProductBffService {
  ...
  prepareOrderTrigger(
    draftId: string,
    input: PrepareProductOrderTriggerInput,
  ): Promise<PrepareProductOrderTriggerResult>;
  triggerOrder(
    draftId: string,
  ...
  getInvite(
    inviteId: string,
    input?: PreviewProductInviteInput,
  ): Promise<ProductInvitePreviewResponse>;
  acceptInvite(
    inviteId: string,
    input: AcceptProductInviteInput,
  ): Promise<InviteWithDraft>;
  rejectInvite(
    inviteId: string,
...
export function createProductBffService(
  options: ProductBffServiceOptions,
): ProductBffService {
  const store = options.store ?? new MemoryProductBffStore();
  const triggerAdapter =
    options.triggerAdapter ??
    options.registrationAdapter ??
    new MemoryProductOrderTriggerBroadcastAdapter();
  const authorizationBuilder =
    options.authorizationBuilder ?? new ProductAuthorizationBuilder();
  const registrarAddress = normalizeAddress(
    options.registrarAddress ??
      triggerAdapter.registrarAddress ??
      DEFAULT_PRODUCT_REGISTRAR_ADDRESS,
    "registrarAddress",
  );
  const registrationCreatorAddress = options.registrationCreatorAddress
    ? normalizeAddress(
        options.registrationCreatorAddress,
  ...
  return {
    ...
    async updateDraft(draftId, input, actorWallet) {
      ...
      const draft: ProductOrderDraftDTO = {
        ...
        ...(input.exportRegion !== undefined
          ? { exportRegion: input.exportRegion }
          : {}),
    ...
    async prepareOrderTrigger(draftId, input) {
      ...
      const submitter = requireTriggerSubmitter(
        zhixu,
        createOrderTrigger,
        participants,
        walletAddress,
      ...
      if (existingRegistration) {
        const expiredPreparedTrigger =
          existingRegistration.status === "prepared" &&
      ...
      if (
        ...
        throw new ProductBffError(
          409,
          "draft_not_ready",
          "order draft is not ready to trigger",
          {
            status: draft.status,
          },
        );
      ...
      const stateMachineAddress = normalizeAddress(
        activeDeployment.stateMachineAddress,
        "activeStateMachineAddress",
      );
      const deploymentId = normalizeBytes32(
      ...
      const triggerStageId = normalizeBytes32(
        createOrderTrigger.triggerStageId,
        "createOrderTrigger.triggerStageId",
      );
    ...
    async triggerOrder(draftId, input) {
      ...
      if (registration.prepareId !== input.prepareId) {
        throw new ProductBffError(
          409,
          "prepare_id_mismatch",
          "trigger prepareId does not match the latest prepared trigger",
          {
      ...
      if (
        ...
        throw new ProductBffError(
          409,
          "trigger_not_prepared",
          "order trigger is not in prepared state",
          {
      ...
      if (!registration.submitter || submitter !== registration.submitter) {
        throw new ProductBffError(
          403,
          "wrong_wallet",
          "connected wallet does not match trigger submitter",
          {
      ...
      assertDraftUsesActiveZhixuVersion(draft, zhixu);
      requireTriggerSubmitter(
        zhixu,
        requireCreateOrderTrigger(zhixu),
        participants,
        submitter,
      );
      assertPrepareNotExpired(registration, now());
      const recovered = await recoverTriggerSigner(
      ...
      if (recovered !== registration.submitter) {
        throw new ProductBffError(
          403,
          "invalid_trigger_signature",
          "trigger signature was not produced by the prepared submitter",
          {
            recovered,
            submitter: registration.submitter,
          },
    ...
    async createInvite(draftId, input, actorWallet) {
      ...
      if (participant.status === "accepted") {
        throw new ProductBffError(
          409,
          "role_already_filled",
          "participant role is already accepted",
          {
    ...
    async listParticipantAssignments(walletAddress) {
      ...
      const participants =
        await store.listAcceptedParticipantsByWallet(normalizedWallet);
      ...
      for (const participant of participants) {
        ...
        const registration = await store.getRegistrationByDraft(
          participant.draftId,
        );
        const permissions = registration
          ? participantPermissionsForWallet(
...
async function requireActiveZhixu(
  ...
  const zhixu = await productService.getZhixu(zhixuId);
  if (!zhixu) {
  ...
  if (versionResolver) {
    ...
    if (!version) {
      throw new ProductBffError(
        409,
        "no_active_version",
        "zhixu has no active Store version",
      );
    }
    if (version.status !== "active") {
      throw new ProductBffError(
        409,
        "version_not_active",
        "zhixu version is not active for new drafts",
        {
...
async function requireRoleParticipant(
  ...
  const participant = (await store.listParticipants(draftId)).find(
    (item) => item.roleSlotId === roleSlotId,
  );
  if (!participant) {
    throw new ProductBffError(
      404,
...
function inviteWithCurrentStatus(
...
function requireValidInviteExpiry(expiresAt: string): void {
  if (Number.isNaN(Date.parse(expiresAt))) {
    throw new ProductBffError(
      400,
      "invalid_expires_at",
      "invite expiresAt must be a valid timestamp",
    );
  }
}
...
async function inviteWalletBinding(
  ...
  const boundParticipant = participants.find(
    (item) =>
...
async function assertWalletCanAcceptInvite(
  ...
  const walletBinding = await inviteWalletBinding(
    store,
    invite,
    participant,
    walletAddress,
...
async function requireRegistrationByDraft(
  ...
  if (!registration) {
    throw new ProductBffError(
      404,
      "trigger_not_found",
      "order draft has no prepared trigger",
    );
  }
  return registration;
}
...
async function refreshDraftStatus(
  ...
  if (
    draft.status === "triggering" ||
    draft.status === "triggered" ||
    draft.status === "failed" ||
  ...
  const status: ProductOrderDraftStatus =
    required.length > 0 &&
    required.every((participant) => participant.status === "accepted")
      ? "ready_to_trigger"
...
function randomOrderId(
  ...
  return hashHex(
    `uvp:product-bff:order:v2:${draft.draftId}:${draft.planId}:${idScope}:${sequence}:${randomUUID()}`,
  );
}

function assertPrepareNotExpired(
  ...
  throw new ProductBffError(
    409,
    "trigger_prepare_expired",
    "prepared trigger typed data has expired",
    {
...
function isPrepareExpired(
  ...
  return (
    ...
    deadlineSeconds * 1000 <= currentTime.getTime()
  );
}

function creatorForDraft(
  draft: ProductOrderDraftDTO,
  fallback: Address,
): Address {
  if (!draft.createdBy) {
...
async function requireActiveStateMachineDeployment(
  ...
  if (!deployment) {
    throw new ProductBffError(
      409,
      "state_machine_deployment_missing",
      "active UVPStateMachine deployment is required for trigger typed data",
    );
  }
  return deployment;
}

function requireCreateOrderTrigger(
  zhixu: ZhixuDetailDTO,
): NonNullable<ZhixuDetailDTO["createOrderTrigger"]> {
  if (!zhixu.createOrderTrigger) {
    throw new ProductBffError(
      409,
      "create_order_trigger_missing",
      "zhixu Product schema must declare createOrderTrigger",
    );
  }
...
function requireTriggerSubmitter(
  ...
  const participant = participants.find(
    (item) => item.roleSlotId === authority.roleSlotId,
  );
  if (
    !participant ||
    participant.status !== "accepted" ||
    ...
    throw new ProductBffError(
      409,
      "trigger_executor_not_accepted",
      "trigger executor role must have an accepted participant wallet",
      {
  ...
  const authorityWallet = normalizeAddress(
    participant.walletAddress,
    "participant.walletAddress",
  );
  if (authorityWallet !== submitter) {
    throw new ProductBffError(
      ...
      "trigger submitter must be the accepted executor for the trigger stage",
      {
        roleSlotId: authority.roleSlotId,
        roleLabel: authority.roleLabel,
...
function resolveTriggerSubmitterAuthority(
  ...
  if (explicitRoleSlotId) {
    return requireTriggerAuthorityRoleSlot(
      zhixu,
      explicitRoleSlotId,
      undefined,
    );
  ...
  const stage = zhixu.stages.find((item) =>
    stageMatchesTriggerStage(item.stageId, triggerStageId),
  );
  if (!stage) {
    throw new ProductBffError(
      409,
      "trigger_authority_unresolved",
      "createOrderTrigger must declare submitterRoleSlotId when triggerStageId cannot be mapped to a Product stage",
      {
        triggerStageId,
      },
    );
  ...
  if (stage.staticExecutorRoleSlotId) {
    return requireTriggerAuthorityRoleSlot(
      zhixu,
      stage.staticExecutorRoleSlotId,
      stage.stageId,
    );
  }
  ...
  const candidateSlots = zhixu.roleSlots.filter((slot) =>
    (slot.capabilityPlugins ?? []).some((plugin) =>
      plugin.stageIds.includes(stage.stageId),
    ),
  );
  ...
  throw new ProductBffError(
    409,
    "trigger_authority_unresolved",
    "trigger stage executor role is ambiguous; createOrderTrigger.submitterRoleSlotId is required",
    {
      triggerStageId,
      stageId: stage.stageId,
...
function requireTriggerAuthorityRoleSlot(
  ...
  if (!roleSlot) {
    throw new ProductBffError(
      409,
      "trigger_authority_unresolved",
      "createOrderTrigger.submitterRoleSlotId must reference an existing role slot",
      {
        roleSlotId,
        ...(stageId ? { stageId } : {}),
      },
...
function stageMatchesTriggerStage(
  stageId: string,
  triggerStageId: Hex,
): boolean {
  try {
...
async function recoverTriggerSigner(
  typedData: unknown,
  signature: string,
): Promise<Address> {
  try {
    return await recoverTriggerOrderFromOutsideSigner(
      typedData as TriggerOrderFromOutsideTypedData,
      signature,
    );
  } catch (error) {
    throw new ProductBffError(
      400,
      "invalid_trigger_signature",
      error instanceof Error ? error.message : "invalid trigger signature",
    );
  }
}
...
function requireAcceptedRequiredParticipants(
  participants: readonly DraftParticipantDTO[],
): void {
  const missing = participants.filter(
    (participant) =>
      participant.required &&
      (participant.status !== "accepted" || !participant.walletAddress),
  );
  if (missing.length > 0) {
    throw new ProductBffError(
      409,
      "required_participant_missing",
      "all required participants must accept before submit",
      {
    ...
    try {
      ...
      if (error instanceof ConfigError) {
        throw new ProductBffError(
          400,
          "invalid_wallet",
          "participant walletAddress must be a valid EVM address",
          {
            participantId: participant.participantId,
            roleSlotId: participant.roleSlotId,
          },
...
function assertDraftUsesActiveZhixuVersion(
  draft: ProductOrderDraftDTO,
  zhixu: ZhixuDetailDTO,
): void {
  const activePlanId = normalizeBytes32(
    zhixu.planPublication.planId,
    "planId",
  );
  const activePlanHash = normalizeBytes32(
    zhixu.planPublication.planHash,
    "planHash",
  );
  ...
  throw new ProductBffError(
    409,
    "version_not_active",
    "order draft plan is no longer active for new registration",
    {
      draftId: draft.draftId,
...
async function broadcastOutsideTrigger(input: {
  ...
  if (registration.status === "failed") {
    throw new ProductBffError(
      ...
      submitResultFromRegistration(
        draft,
        await input.store.listParticipants(draft.draftId),
        registration,
      ),
    );
  }
...
async function safeBroadcastOutsideTrigger(
  ...
  try {
    if (!registration.signature) {
      throw new ProductBffError(
        400,
        "trigger_signature_missing",
        "business wallet signature is required",
      );
    }
    if (
    ...
    return await triggerAdapter.broadcastOutsideTrigger({
      ...
      ...(registration.stateMachineAddress
        ? { stateMachineAddress: registration.stateMachineAddress }
        : {}),
    ...
    return {
      status: "failed",
      errorCode:
        error instanceof ProductBffError
          ? error.code
          : "trigger_order_adapter_failed",
      errorMessage:
        error instanceof Error ? error.message : "trigger order adapter failed",
      retryable: true,
    };
  }
}
...
function preparedTriggerFromRegistration(
  ...
  return {
    ...
    orderId: registration.orderId,
    expiresAt: new Date(
      Number.parseInt(registration.deadline, 10) * 1000,
    ).toISOString(),
    submitter: registration.submitter,
...
function registrationDtoFromRecord(
  registration: ProductOrderTriggerRecord,
): ProductOrderTriggerDTO {
  const {
    creator: _creator,
...
function defaultRegistrationReconcileFields(
  registration: ProductOrderTriggerRecord,
): TxReconcileFields {
  if (
    registration.reconcileStatus ||
    registration.receiptStatus ||
    registration.projectionStatus
  ) {
    return {};
  }
  if (registration.status === "confirmed") {
