...
export interface DockedSignalAutomationService {
  processProjection(snapshot: ProjectionSnapshot): Promise<DockedSignalAutomationSummary>;
}
...
export function createDockedSignalAutomationService(
  ...
  return {
    async processProjection(snapshot): Promise<DockedSignalAutomationSummary> {
      if (
        !options.config.enabled ||
        !options.dockingModuleAddress ||
        options.config.maxCandidatesPerRun <= 0
      ) {
  ...
  for (const localOrder of Object.values(snapshot.stateMachineOrders)) {
    for (const link of Object.values(localOrder.dockedOrderLinks)) {
      ...
      for (const binding of Object.values(link.signalBindings)) {
        scannedBindings += 1;
        if (
          hasSignal(localOrder, binding.localSourceId, binding.localSignalId)
        ) {
          continue;
        }
        const linkedSignal = signalFor(
          linkedOrder,
        ...
        const candidateId = dockedSignalCandidateId(
          localOrder,
          binding,
          linkedSignal,
        );
...
function reconcileSubmitted(
  ...
  for (const candidateId of [...submitted]) {
    if (!projectionStillNeedsCandidate(snapshot, candidateId)) {
